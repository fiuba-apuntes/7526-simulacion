...
\hline 
 & \texttt{START} & \texttt{n} & Simular $n$ transacciones & $n=$ cantidad de transacciones & \tabularnewline
\hline
\endhead
\hline 
 & \texttt{GENERATE} & \texttt{{[}t{]},{[}d{]},{[}o{]},{[}c{]},p{]}} & Arribo de transacciones independientes entre sí cada cierto tiempo
$t\pm d$ o $t\times d$ & \begin{itemize}
\item $t=$ valor medio entre arribos
\item Si $d\in N$, es el desvío máximo permitido. Si $d$ es una \texttt{FUNCTION},
se multiplica $t\times d$ para calcular el próximo nacimiento
\item $o=$ instante en que se genera la primera transacción
\item $c=$ cantidad máxima de transacciones a generar
\item $p=$ prioridad de la transacción a generar\end{itemize}
 & \begin{itemize}
\item $t=0$
\item $d=0$
\item $o=1$
\item $c=\infty$
\item $p=0$ (prioridad mínima)\end{itemize}
\tabularnewline
\hline 
 & \texttt{ADVANCE} & \texttt{t{[},d{]}} & Realización de una tarea que dura cierto tiempo $t\pm d$ o $t\times d$ & \begin{itemize}
\item $t=$ valor medio de la duración de la tarea (constante o SNA)
\item Si $d\in N$, es el desvío máximo permitido. Si $d$ es una \texttt{FUNCTION},
se multiplica $t\times d$ para calcular la duración. \emph{No puede
ser referencia a FUNCTION.}
\item Debe cumplirse que $t\geq d$\end{itemize}
 & \begin{itemize}
\item $t=0$
\item $d=0$\end{itemize}
\tabularnewline
\hline 
 & \texttt{TERMINATE } & \texttt{{[}n{]}} & Salida de $n$ transacciones. Si $n$ no se especifica, no se contabiliza
