...
\begin{itemize}
  ...
  \item $t=$ tipo de la función ($C=$continua, $D=$discreta, $E=$discreta de atributos numéricos)
    \footnote{Debe cumplirse que $x_{1}\leq x_{2}\leq...\leq x_{n}$.
      Si es \textbf{continua}, y $x_{i}<x<x_{i+1}$ se interpola linealmente entre $y_{i}$ e $y_{i+1}$.
      Si es \textbf{discreta}, y $x_{i}<x<x_{i+1}$, se devuelve $y_{i+1}$ (los $y$ pueden ser rótulos).
      Si es \textbf{discreta de atributos numéricos}, es como la discreta pero las $y$ deben ser SNAs.}
